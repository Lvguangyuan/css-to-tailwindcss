...
<template>
  <div class="min-h-screen flex flex-col bg-gradient-to-b from-slate-50 to-slate-100">
    ...
    <main
        class="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 p-6 lg:flex-row"
    >
      <!-- ─── Configuration panel (left side) ─── -->
      <aside
          class="w-full shrink-0 rounded-2xl bg-white/60 shadow backdrop-blur-md ring-1 ring-black/5 px-6 py-4 lg:w-56"
      >
        ...
        <div class="flex items-center justify-between">
          <span class="text-sm text-gray-700">px → rem</span>

          <!-- accessible toggle -->
          <label class="relative inline-flex cursor-pointer items-center">
            <input
                type="checkbox"
                v-model="pxToRemEnabled"
                class="peer sr-only"
            />
            <!-- track -->
            <div
                class="h-5 w-9 rounded-full bg-gray-300 transition-colors peer-checked:bg-indigo-600"
            ></div>
            <!-- knob -->
            <div
        ...
        >
          <label class="whitespace-nowrap text-gray-700">1&nbsp;rem&nbsp;=</label>
          <input
              v-model.number="remBase"
              type="number"
              min="1"
              class="w-20 rounded border px-2 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-400"
          />
      ...
      <!-- ─── Cards area ─── -->
      <div class="flex-1 grid gap-6 lg:grid-cols-2">
        <!-- Card – Input -->
        <article
...
/* ─── shared debounce for cssInput, remBase, pxToRemEnabled ─── */
let debounceId: ReturnType<typeof setTimeout>
watch([cssInput, remBase, pxToRemEnabled], () => {
...
function extractAll(css: string) {
  ...
  while ((m = re.exec(css))) {
    ...
    if (pxToRemEnabled.value) {
      classes = classes.replace(/(\d*\.?\d+)px/g, (_, num) => {
        const rem = parseFloat(num) / (remBase.value || 16)
